...
public interface BarcodesApi {
    ...
    @ApiOperation(value = "Creates a barcode", notes = "Creates a single barcode with the given input parameters (prefix, info)", response = Barcode.class, tags = {
            "barcode", })
    ...
    ResponseEntity<Barcode> createSingleBarcode(
            @ApiParam(value = "Input parameters of the Barcode object that needs to be created", required = true) @RequestBody SingleBarcodePayload body);
            
    @ApiOperation(value = "Finds Barcode by prefix", notes = "Find the latest Barcode for the given prefix.", response = Barcode.class, tags = {
            "barcode", })
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Successful operation", response = Barcode.class),
            @ApiResponse(code = 404, message = "Barcode not found", response = Barcode.class) })
    @RequestMapping(value = "/barcodes/findByPrefix", produces = { "application/json" }, method = RequestMethod.GET)
    ResponseEntity<Barcode> findBarcodeByPrefix(
